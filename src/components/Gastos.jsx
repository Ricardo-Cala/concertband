...
export default function Gastos({ conciertos, amigos }) {
  ...
  const cargarDatos = async () => {
    const [g, p, a] = await Promise.all([
      ...
      supabase.from('pagos').select('*, amigos(nombre, iniciales, color)').eq('gasto_id', 'placeholder'),
    ...
    if (g.data) {
      ...
      if (g.data.length > 0) {
        ...
        const { data: pagoData } = await supabase
          ...
          .in('gasto_id', ids)
        if (pagoData) setPagos(pagoData)
      } else {
        setPagos([])
      }
    }
    if (a.data) setAsistentes(a.data)
  }
  ...
  const guardarGasto = async () => {
    ...
    if (gasto) {
      const otrosAsistentes = asistentes.filter(a => a.amigo_id !== form.comprador_id)
      if (otrosAsistentes.length > 0) {
        await supabase.from('pagos').insert(
          otrosAsistentes.map(a => ({
            gasto_id: gasto.id,
  ...
  const Avatar = ({ amigo, size = 32 }) => (
    <div style={{
      width: size, height: size, borderRadius: '50%',
      background: amigo?.color, color: 'white',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontSize: size === 32 ? 11 : 9, fontWeight: 500, flexShrink: 0
    }}>{amigo?.iniciales}</div>
  )
    ...
    <div style={{ padding: 16 }}>
      ...
      {concierto && (
        <>
          ...
          {gastos.map(g => {
            const pagosDeEsteGasto = pagos.filter(p => p.gasto_id === g.id)
            const pendientes = pagosDeEsteGasto.filter(p => !p.pagado)
            const cobrados = pagosDeEsteGasto.filter(p => p.pagado)
            return (
              <div key={g.id} style={{ background: 'white', borderRadius: 12, padding: 14, marginBottom: 12, border: '1px solid #eee' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <button onClick={() => borrarGasto(g.id)} style={{ background: 'none', border: 'none', fontSize: 12, color: '#ccc', cursor: 'pointer' }}>🗑️</button>
                </div>

                {pendientes.length > 0 && (
                  <div style={{ marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: '#A32D2D', marginBottom: 6 }}>DEBEN PAGAR</div>
                    {pendientes.map(p => (
                ...
                {cobrados.length > 0 && (
                  <div>
                    <div style={{ fontSize: 11, color: '#3B6D11', marginBottom: 6 }}>YA PAGARON</div>
                    {cobrados.map(p => (
          ...
          {mostrarForm && (
            <div style={{ background: 'white', borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid #7F77DD' }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input type='number' value={form.precio_entrada} onChange={e => setForm(f => ({ ...f, precio_entrada: e.target.value }))}
                  placeholder='Ej: 65'
